...
const MARKER = 'CITAJUSTA_DESKTOP_AGENDA_READER_V1';
const ROLE = 'DEMO_DESKTOP_AGENDA_READER';
const PERMISSIONS = ['agenda.read'] as const;
...
function compatible(row: object, expected: object, label: string) {
  ...
  if (Object.entries(expected).some(([key, value]) => values.get(key) !== value)) {
    throw new DevelopmentError(`Desktop agenda: ${label} incompatible; no se sobrescribe.`);
  }
}
export function parseDesktopAgendaReaderEmail(email: string | undefined): string {
  const parsed = z.string().trim().email().max(254).safeParse(email);
  if (!parsed.success) throw new DevelopmentError('Configura DESKTOP_AGENDA_READER_EMAIL con el email válido de una cuenta ACTIVE existente.');
  return parsed.data.toLowerCase();
}

export async function seedDesktopAgendaReader(prisma: Pick<PrismaClient, '$transaction'>, options: {
  ...
  const email = parseDesktopAgendaReaderEmail(options.email);
  for (let attempt = 0; ; attempt++) {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const role = { id: uuid('role'), code: ROLE, name: 'Demo local Desktop agenda', description: MARKER, scope: 'INSTITUTION' as const, active: true };
        const roles = await tx.role.findMany({ where: { OR: [{ id: role.id }, { code: role.code }] } });
        if (roles.length > 1) throw new DevelopmentError('Colisión de identidad del rol Desktop agenda.');
        ...
        if (links.some((link) => !PERMISSIONS.some((code) => code === link.permission.code))) {
          throw new DevelopmentError('El rol Desktop agenda contiene permisos ajenos.');
        }
        ...
        if (grants.length > 1) throw new DevelopmentError('El rol Desktop agenda tiene asignaciones ajenas o duplicadas.');
        ...
        for (const code of PERMISSIONS) {
          ...
          if (matches.length > 1) throw new DevelopmentError('Colisión de identidad de permiso Desktop agenda.');
