...
export async function bootstrapWaitlist(prisma: Pick<PrismaClient, '$transaction'>, institutionId?: string) {
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      return await prisma.$transaction((tx) => provisionWaitlistCatalog(tx, institutionId), { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
    } catch (error) {
...
// Reusable dentro de la transacción del seed; el bootstrap público conserva sus reintentos.
export async function provisionWaitlistCatalog(tx: Prisma.TransactionClient, institutionId?: string) {
