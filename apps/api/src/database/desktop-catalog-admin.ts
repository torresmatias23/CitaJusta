...
const MARKER = 'CITAJUSTA_DESKTOP_CATALOG_ADMIN_V1';
const ROLE = 'DEMO_DESKTOP_CATALOG_ADMIN';
const PERMISSIONS = ['branches.read', 'branches.create', 'branches.update', 'services.read', 'services.create', 'services.update'] as const;
...
function compatible(row: object, expected: object, label: string) {
  ...
  if (Object.entries(expected).some(([key, value]) => values.get(key) !== value)) {
    throw new DevelopmentError(`Desktop catálogo: ${label} incompatible; no se sobrescribe.`);
  }
}
export function parseDesktopCatalogAdminEmail(email: string | undefined): string {
  const parsed = z.string().trim().email().max(254).safeParse(email);
  if (!parsed.success) throw new DevelopmentError('Configura DESKTOP_CATALOG_ADMIN_EMAIL con el email válido de una cuenta ACTIVE existente.');
  return parsed.data.toLowerCase();
}

export async function seedDesktopCatalogAdmin(prisma: Pick<PrismaClient, '$transaction'>, options: {
  ...
  const email = parseDesktopCatalogAdminEmail(options.email);
  for (let attempt = 0; ; attempt++) {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const role = { id: uuid('role'), code: ROLE, name: 'Demo local Desktop catálogo', description: MARKER, scope: 'INSTITUTION' as const, active: true };
        const roles = await tx.role.findMany({ where: { OR: [{ id: role.id }, { code: role.code }] } });
        if (roles.length > 1) throw new DevelopmentError('Colisión de identidad del rol Desktop catálogo.');
        ...
        if (links.some((link) => !PERMISSIONS.some((code) => code === link.permission.code))) {
          throw new DevelopmentError('El rol Desktop catálogo contiene permisos ajenos.');
        }
        ...
        if (grants.length > 1) throw new DevelopmentError('El rol Desktop catálogo tiene asignaciones ajenas o duplicadas.');
        ...
        for (const code of PERMISSIONS) {
          ...
          if (matches.length > 1) throw new DevelopmentError('Colisión de identidad de permiso Desktop catálogo.');
