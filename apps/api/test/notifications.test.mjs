...
import {
  ...
  emptyInput,
} from '../dist/notifications/notifications.schemas.js';
...
function event(type = 'APPOINTMENT_BOOKED') {
  ...
  return {
    ...
    branchId: null,
    type,
    resourceType: type.startsWith('APPOINTMENT')
      ? 'APPOINTMENT'
    ...
    data: [
      ...
      ? {
          ...
          ...(type === 'OFFER_CREATED'
            ? { expiresAt: '2029-12-31T12:00:00.000Z' }
            : {}),
        }
      : {},
  };
...
test('eight typed events create only controlled data through the supplied transaction', async () => {
  for (const type of types) {
    ...
    assert.ok(content.title && content.message);
    assert.ok(
      ['/mis-citas', '/lista-de-espera', '/ofertas'].includes(content.path),
    );
    assert.deepEqual(
      Object.keys(content).sort(),
      ['message', 'path', 'title'].sort(),
    );
  }
});

test('notification content uses institutional timezone', () => {
  ...
  assert.ok(content.message.includes('America/Santiago'));
  assert.equal(content.message.includes('(UTC)'), false);
});
...
test('helper rejects unknown/private content, incoherent keys and resources before any write', async () => {
  ...
  for (const invalid of [
    { ...input, type: 'EMAIL' },
    { ...input, title: 'arbitrary' },
    { ...input, data: { ...input.data, email: 'secret' } },
    { ...input, data: { startsAt: 'bad' } },
    { ...input, branchId: randomUUID() },
    { ...input, resourceType: 'OFFER' },
    {
      ...input,
      dedupeKey: `APPOINTMENT_BOOKED:${randomUUID()}`,
    },
  ]) {
    await assert.rejects(
      NotificationsService.create(
        {
          notification: {
            createMany: () => assert.fail('must not write'),
          },
        },
        invalid,
      ),
  ...
  assert.doesNotThrow(() =>
    parseNotification({
      ...
      dedupeKey: `APPOINTMENT_CANCELLED:${randomUUID()}`,
    }),
  );
});
...
test('strict query, default/max limit and canonical timestamp/UUID cursor', () => {
  assert.deepEqual(parseNotificationQuery({}), { limit: 50 });
  assert.equal(parseNotificationQuery({ limit: '100' }).limit, 100);
  ...
  const cursor = encodeNotificationCursor(row);

  assert.deepEqual(decodeNotificationCursor(cursor), row);
...
test('reads scope list/count to principal, use institutional timezone and strip private fields', async () => {
  ...
  assert.equal(
    'institutionId' in result.data[0],
    false,
  );

  assert.ok(
    result.data[0].message.includes('America/Santiago'),
  );

  assert.equal(
    result.data[0].message.includes('(UTC)'),
    false,
  );
});
...
test('controller requires principal and rejects functional body/query on read', () => {
  const controller = new NotificationsController({
    read: () => assert.fail('must not call'),
  });

  assert.throws(
    () => controller.findMine({}, undefined, {}),
