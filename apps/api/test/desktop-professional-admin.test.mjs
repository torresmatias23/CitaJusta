...
import { parseDesktopProfessionalAdminEmail, seedDesktopProfessionalAdmin } from '../dist/database/desktop-professional-admin.js';
...
const codes = ['professionals.read', 'professionals.create', 'professionals.update', 'branches.read', 'services.read'];
function fixture() {
  ...
  return { state, run: (patch = {}) => seedDesktopProfessionalAdmin(prisma, { ...options, ...patch }) };
}
...
test('email absent/invalid rejected and normalization matches Auth', async () => {
  ...
  assert.equal(parseDesktopProfessionalAdminEmail(' Desktop@Example.Test '), options.email);
});
...
test('creates exactly five permissions and institutional grant; replay preserves user and all RBAC rows', async () => {
  ...
  assert.equal(role.code, 'DEMO_DESKTOP_PROFESSIONAL_ADMIN'); assert.equal(role.scope, 'INSTITUTION'); assert.equal(role.active, true);
  assert.equal(f.state.links.length, 5);
...
test('compatible existing permissions reused; unrelated user roles untouched', async () => {
  ...
  await f.run();
  assert.equal(f.state.permissions.length, 5);
...
test('coexists with HU-029 without changing its six permissions, role or grant', async () => {
  ...
  assert.equal(f.state.permissions.length, 9);
...
test('transaction rolls back partial writes and retries only recognized conflict once', async () => {
  ...
  await f.run(); assert.equal(f.state.transactions, 3); assert.equal(f.state.links.length, 5);
