...
import type { ApiClient, RequestOptions } from './http-client.ts';
  ...
  return {
    ...
    async logout() {
      ...
      try { await revoke(token); }
        ...
        if (generation === expected) update({ status: 'anonymous', user: null, error: 'Sesión cerrada localmente. No pudimos confirmar la revocación en el servidor.' });
      }
    },
  };
}
