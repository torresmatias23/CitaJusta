...
### HU-013: administración institucional de profesionales

HU-030 incorpora Desktop Profesionales y dos GET administrativos estáticos, anteriores a `:professionalId`: `/api/v1/professionals/administration` (`professionals.read`) lista profesionales ACTIVE/INACTIVE/SUSPENDED de la institución sin eliminados; `/api/v1/professionals/eligible-users?email=<email>` (`professionals.create`) busca una cuenta ACTIVE existente por email exacto, validado/normalizado como Auth. Ambos requieren guards/contexto y grants GLOBAL/INSTITUTION, rechazan contexto BRANCH y no cambian los GET públicos. La elegibilidad excluye perfiles previos de esa institución, incluso eliminados; causas no elegibles comparten 404. DTO de cuenta limitado a id/email/nombres/apellidos. Permisos provisionados explícitamente, nunca autoasignados. Desktop reutiliza POST/PATCH HU-013 y catálogos HU-029, sin crear cuentas ni modificar credenciales; detalle y validación en [README Desktop](apps/desktop/README.md).

- `POST /api/v1/professionals` requiere `professionals.create`; `PATCH /api/v1/professionals/:professionalId` requiere `professionals.update`. Bearer token, `x-institution-id` obligatorio y contexto sin `x-branch-id`: ambas operaciones afectan al profesional institucional completo. Reutiliza `AccessTokenGuard`, `AuthorizationContextGuard`, `PermissionsGuard` y los grants RBAC GLOBAL/INSTITUTION; un rol limitado a una sede no basta.
- Alta: `{ userId, branchIds, serviceIds, internalCode?, titleOrFunction?, description?, status? }`. Usuario existente ACTIVE/no eliminado y al menos una sede y un servicio activos/no eliminados de la institución del contexto. `User` es una identidad global: no se exige un rol institucional previo al usuario objetivo, ni se le otorgan roles, permisos o credenciales al registrarlo como profesional. No se crea ni modifica la cuenta.
- PATCH admite los mismos campos salvo `userId`, que queda inmutable para conservar identidad histórica. Parcial, no vacío; sin query ni propiedades desconocidas. Campos de texto opcionales admiten null explícito para limpiarse; códigos se recortan sin cambiar mayúsculas. `status`: `ACTIVE`, `INACTIVE`, `SUSPENDED`. La unicidad existente `(institutionId, userId)` y `(institutionId, internalCode)` incluye profesionales inactivos/eliminados; varios códigos null siguen siendo válidos según PostgreSQL.
- `branchIds` y `serviceIds`: hasta 100 UUID únicos por array. En PATCH, omitidos conservan asociaciones; enviados reemplazan el conjunto activo, y `[]` desactiva todas. Se hace upsert por PK compuesta y se conservan filas retiradas, `createdAt` y `customDurationMinutes`. Las asociaciones son independientes: no crean vínculos ServiceBranch ni garantizan disponibilidad para cada combinación; los filtros operativos existentes siguen comprobando esas relaciones.
- `201` en POST y `200` en PATCH: `{ data: { id, userId, internalCode, titleOrFunction, description, status, createdAt, updatedAt, branchIds, serviceIds } }`. Timestamps ISO UTC; asociaciones activas ordenadas por UUID y limitadas a la institución. `userId` identifica exclusivamente la cuenta vinculada en este DTO administrativo; no se devuelven email, credenciales, roles ni relaciones Prisma completas. Los GET públicos mantienen exactamente su contrato anterior.
- Revalidación de permisos vigentes, institución activa y relaciones dentro de una transacción Serializable. Reintento completo una sola vez ante conflicto transaccional reconocido; UNIQUE genera `409`. Errores: `400` contrato/contexto inválido, `401` sin autenticación, `403` sin permiso institucional suficiente, `404` usuario/recurso/relación no disponible o fuera de alcance, `409` duplicado/conflicto persistente y `500` inesperado sin detalles internos. El guard puede rechazar con `403` antes de validar un contexto ausente.
- Provisionamiento explícito: crear permisos `code=professionals.create/update`, `module=professionals`, `action=create/update`; vincularlos mediante RolePermission a roles autorizados y asignar UserRole al ámbito institucional correcto. No hay bootstrap automático de permisos.
- Se conservan citas, horarios, disponibilidad materializada e historial; no hay DELETE físico ni migraciones. Un cambio de asociaciones actualiza también `Professional.updatedAt`. El criterio de auditoría del Backlog HU-013/RF-036 no queda satisfecho íntegramente: no existe bitácora administrativa reutilizable con actor/valores anteriores. Timestamps y asociaciones conservadas no equivalen a auditoría completa; esa infraestructura sigue pendiente, sin crear un subsistema paralelo en HU-013.
- E2E: `npm run test:e2e:professionals -w @citajusta/api`, secuencial con los otros E2E porque comparte fixtures protegidos. Requiere PostgreSQL local permitido, esquema vigente y bootstrap de estados de cita existente. Comprueba RBAC, aislamiento, UNIQUE/concurrencia, rollback SQL de alta y actualización, preservación histórica y lecturas compatibles.

### HU-014: administración de disponibilidad y bloqueos

- `POST /api/v1/availability` (`availability.create`), `PATCH /api/v1/availability/:availabilityId` (`availability.update`) y `POST /api/v1/availability/blocks` (`availability.block`). Bearer token y `x-institution-id` obligatorios; los guards existentes y la transacción revalidan grants GLOBAL/INSTITUTION o BRANCH con `x-branch-id` coincidente. Provisionar explícitamente esos permisos (`module=availability`, `action=create/update/block`), RolePermission y UserRole; no se conceden permisos automáticamente.
- Alta de disponibilidad: `{ branchId, professionalId, serviceId, attentionPointId?, startsAt, endsAt }`. Relaciones y asignaciones activas/coherentes dentro de la institución. Timestamps con zona explícita y precisión de minutos; intervalo futuro, de un mismo día institucional, sin atravesar un cambio de offset horario. La zona proviene de Institution.timeZone. Duración de cupo: ProfessionalService.customDurationMinutes o Service.durationMinutes; debe dividir íntegramente el intervalo, máximo 500 cupos. Se crea Availability MANUAL, capacidad individual 1, y se materializan AgendaSlot en la misma transacción; no se añade un motor de horarios recurrentes.
- PATCH admite `{ startsAt, endsAt, active? }` o `{ active: false }`, sin cambiar profesional/servicio/sede/punto. Reactivar exige fechas explícitas y `active: true`. Cupos sin uso se conservan como EXPIRED; los intervalos exactos reutilizables se actualizan con lockVersion y los nuevos se materializan. Cualquier cita, reasignación, slot RESERVED/RELEASED/BLOCKED o bloqueo temporal vigente impide editar/desactivar con `409`; no se borran citas, cupos ni historial.
- Solapes: un profesional no admite intervalos simultáneos; intervalos adyacentes sí. Profesionales distintos pueden compartir punto de atención hasta AttentionPoint.capacity. También se comprueban cupos retenidos de disponibilidades inactivas. Serializable y un único reintento completo ante conflicto reconocido protegen las comprobaciones y escrituras; las restricciones UNIQUE existentes se conservan.
- Bloqueo: `{ branchId, professionalId?, attentionPointId?, startsAt, endsAt, type?, reason? }`. Sede obligatoria; sin filtros opcionales afecta toda la sede, con ambos afecta su intersección. `type`: VACATION, LEAVE, MEETING, MAINTENANCE, MANUAL (default), OTHER. Intervalo futuro y actor exclusivamente autenticado. Duplicado de ámbito/fechas devuelve `409`; otros bloqueos superpuestos son acumulativos. Máximo 5000 cupos materializados afectados. Si hay una cita vinculada, incluso cancelada, o un cupo RESERVED/RELEASED/con reasignación, se rechaza todo con `409`.
- Un bloqueo cambia sólo cupos AVAILABLE a BLOCKED mediante actualización condicional y lockVersion. Crear disponibilidad posteriormente consulta ScheduleBlock y Holiday antes de materializar: los cupos afectados nacen BLOCKED. Los GET públicos y HU-004 mantienen su contrato, excluyen BLOCKED y no permiten reservarlo. No se modifica el flujo de ofertas/reasignación ni se devuelve RELEASED a AVAILABLE.
- ScheduleBlock no tiene estado activo, borrado lógico ni datos de retiro: esta HU no ofrece desactivación/retiro de bloqueos ni DELETE físico; su intervalo permanece registrado. Tampoco administra Holiday ni ProfessionalSchedule. El bloqueo conserva createdByUserId/createdAt y Availability sus timestamps; no equivalen a una auditoría administrativa completa, cuya infraestructura continúa pendiente.
- Respuestas `{ data: ... }`, `201` en POST y `200` en PATCH. DTO disponibilidad: `id`, `professionalId`, `serviceId`, `branchId`, `attentionPointId`, `date`, `startTime`, `endTime`, `timeZone`, `active`, `capacity`, `origin`, `createdAt`, `updatedAt`, `slots: [{ id, startsAt, endsAt, status }]`. DTO bloqueo: `id`, `branchId`, `professionalId`, `attentionPointId`, `type`, `reason`, `startsAt`, `endsAt`, `createdAt`. Instantes ISO UTC; date/horas describen el día institucional. Sin institutionId, actor interno, lockVersion ni relaciones completas. Rechaza query y campos desconocidos.
...
- Bearer token y contexto `x-institution-id` obligatorios, con los guards existentes y permiso `agenda.read`. Provisionar explícitamente Permission (`code=agenda.read`, `module=agenda`, `action=read`), RolePermission y UserRole en ámbito GLOBAL/INSTITUTION/BRANCH. No hay bootstrap automático. Con `x-branch-id` la consulta queda siempre limitada a esa sede; intentar otra devuelve `404`, igual que filtros de recursos ajenos. Sin permiso: `403`; sin autenticación: `401`; request/contexto inválido: `400` (el guard puede rechazar antes con `403`).
- Consulta únicamente Appointment con `deletedAt=null`, institución y relaciones sede/servicio/profesional institucionalmente coherentes. No filtra actividad actual de catálogos: las citas históricas permanecen visibles. Día definido por startsAt en Institution.timeZone, usando IANA/Intl como HU-014; límites reales del día, también durante DST, no intervalos UTC fijos de 24 horas. Zona inválida: `503`. Orden `startsAt ASC, id ASC`.
- Respuesta `200 { data: [...] }`: `id`, `startsAt`, `endsAt` ISO UTC, `origin`, `status: { code, name }`, `branch/service: { id, name }`, `professional: { id, firstNames, lastNames, titleOrFunction }`, `user: { id, firstNames, lastNames }`. Identidad mínima del usuario sólo para operación autorizada; sin email, credenciales, notas, permisos ni relaciones completas. Sin slots libres ni paginación en esta iteración.
- Módulo dedicado read-only: selects explícitos, sin escrituras ni transacciones Serializable. No modifica los módulos anteriores, Prisma ni dependencias. E2E: `npm run test:e2e:agenda -w @citajusta/api`, secuencial con las demás suites que comparten fixtures protegidos; requiere PostgreSQL local y catálogos AGENDADA/CANCELADA existentes.

### HU-017: registrar asistencia e inasistencia

- `POST /api/v1/appointments/:appointmentId/attendance`, body estricto `{ "status": "ATENDIDA" }` o `{ "status": "INASISTENCIA" }`. UUID válido; sin query funcional, identidades, motivo libre ni campos adicionales. Responde `200 { data: { id, status: { code, name }, startsAt, endsAt, branch: { id, name }, service: { id, name }, professional: { id, firstNames, lastNames, titleOrFunction } } }`, con timestamps ISO UTC.
- Bearer token, contexto institucional y permiso `appointments.attendance`; provisionar explícitamente Permission (`module=appointments`, `action=attendance`), RolePermission y UserRole en el ámbito autorizado. Contexto BRANCH queda limitado a su sede. Reutiliza los tres guards y revalida dentro de la transacción actor activo, permisos vigentes y relaciones institucionales/cupo coherentes. Cita inexistente, eliminada o fuera de alcance: `404` no divulgativo.
- Sólo `AGENDADA → ATENDIDA/INASISTENCIA`; mismo resultado devuelve `200` sin nueva escritura, timestamp ni historial. CANCELADA, resultados contradictorios y otros estados incompatibles devuelven `409`. No existe corrección/reapertura administrativa en HU-017. La especificación no define una ventana temporal de registro: no se añade una restricción nueva por fecha.
- Bootstrap existente: `npm run bootstrap:appointment-status -w @citajusta/api`. Añade ATENDIDA e INASISTENCIA con `active=true`, `isFinal=true`, `allowsCancellation=false`, `allowsConfirmation=false`. Conserva IDs, nombres y configuración existente; una configuración de asistencia incompatible falla sin sobrescribirla. AGENDADA/CANCELADA mantienen su comportamiento. Catálogo destino ausente/incompatible en el endpoint: `503`; no se provisiona desde la solicitud HTTP.
- Transacción Serializable con actualización condicional y un reintento completo sólo ante conflictos transaccionales reconocidos. Appointment y un único AppointmentHistory se confirman juntos: estado anterior/nuevo, usuario anterior/nuevo sin cambio, actor autenticado y reason `ATTENDANCE_RECORDED` o `NO_SHOW_RECORDED`. No escribe AgendaSlot, lockVersion, ofertas ni reasignaciones. Agenda y Mis citas leen el resultado persistido sin cambios especiales.
- E2E: `npm run test:e2e:attendance -w @citajusta/api`; requiere PostgreSQL local permitido y se ejecuta secuencialmente con los demás fixtures protegidos. Incluye carreras equivalentes/contradictorias con lecturas SQL sincronizadas y rollback tras INSERT real de historial.

### HU-018: política institucional de reasignación

- `GET /api/v1/reassignment-policy` requiere `reassignments.policy.read`; `POST` requiere `reassignments.policy.update`. Bearer token y `x-institution-id` obligatorios; contexto BRANCH rechazado incluso con permiso. Provisionar explícitamente Permission (`module=reassignments.policy`, `action=read/update`), RolePermission y UserRole GLOBAL/INSTITUTION; no hay bootstrap automático.
- POST estricto: `{ "rankingStrategy": "PRIORITY_THEN_WAITING", "offerTtlMinutes": 10 }`, ambos obligatorios, TTL entero **1..60**. Sólo estrategias PRIORITY_THEN_WAITING (nivel ASC, ingreso ASC, UUID ASC: ranking legacy exacto) y WAITING_THEN_PRIORITY (ingreso ASC, nivel ASC, mismo UUID ASC). No cambia elegibilidad, exclusiones, prioridades ni la representación de snapshots/scoring existente.
- Cada configuración diferente crea una versión y devuelve `201`; configuración idéntica a current devuelve `200` sin escrituras. Primera configuración explícita crea versión 1 aunque coincida con el default. Respuesta `{ data: { source: "CONFIGURED", id, version, rankingStrategy, offerTtlMinutes, createdByUserId, createdAt } }`; actor obtenido del principal/contexto y fecha ISO UTC. Sin política, GET devuelve `{ data: { source: "DEFAULT", id: null, version: null, rankingStrategy: "PRIORITY_THEN_WAITING", offerTtlMinutes: <WAITLIST_OFFER_TTL_MINUTES efectivo> } }`, sin escribir ni limitar retroactivamente el TTL legacy al rango del nuevo contrato.
- ReassignmentPolicy conserva versiones históricas; Institution guarda un único puntero current. UNIQUE(institutionId, version), FKs compuestas tenant/policy, checks de versión positiva y TTL y triggers impiden UPDATE de versiones o cambio de la asociación original de Reassignment. Sin endpoint de edición/borrado histórico. POST usa Serializable, inserción y cambio condicional de puntero atómicos y un reintento completo ante conflicto reconocido, incluyendo únicamente P2002 de la restricción de versión institucional.
- HU-009 resuelve current dentro de la transacción, persiste policyId y aplica ranking/TTL; sin política conserva policyId=null y comportamiento legacy. HU-011 usa la política del proceso, nunca current; los procesos null mantienen el TTL global legacy. Cambiar current no modifica procesos, candidatos, snapshots ni ofertas anteriores. HU-016 añade `policy: { id, version, rankingStrategy, offerTtlMinutes }`, o `null` para legacy. Sin cambios en aceptación/transferencia ni AgendaSlot.
- Migración aditiva `20260915230838_reassignment_policy`, Prisma 7.10.0; referencias nuevas nullable, sin backfill ficticio. E2E crítico: `npm run test:e2e:reassignment-policy -w @citajusta/api`, secuencial con E2E reassignments por fixtures compartidos. Conflicto persistente: `409`; scope/permisos: `403`; institución no disponible: `404`; input inválido: `400`; errores inesperados: `500` sin detalles internos.
